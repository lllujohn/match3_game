...
static const SDL_Color kGemHighlight[MAX_GEM_TYPES] = {
    {255, 170, 170, 160}, {160, 230, 210, 160}, {160, 200, 245, 160}, {255, 225, 150, 160},
    {210, 180, 235, 160}, {255, 200, 160, 160}, {255, 255, 255, 160},
};
...
static TTF_Font *load_font_any_path(int size) {
    static const char *PATHS[] = {
        /* 首选自带字体资源 */
        "assets/fonts/NotoSans-Regular.ttf", "assets/fonts/DejaVuSans.ttf",
        "../assets/fonts/NotoSans-Regular.ttf", "../assets/fonts/DejaVuSans.ttf",
        /* macOS */
...
/* 逐行扫描填充圆形 */
static void fill_circle(SDL_Renderer *r, int cx, int cy, int radius, SDL_Color color) {
    set_color(r, color);
...
/* 填充四分之一圆 */
static void fill_quarter_circle(SDL_Renderer *r, int cx, int cy, int radius, int quad) {
...
/* 填充圆角矩形（无边框） */
static void fill_rounded_rect(SDL_Renderer *r, int x, int y, int w, int h, int cr,
                              SDL_Color color) {
    set_color(r, color);
    set_blend(r, SDL_BLENDMODE_BLEND);
...
/* 画圆角矩形边框 */
static void draw_rounded_rect_outline(SDL_Renderer *r, int x, int y, int w, int h, int cr,
...
static void draw_text_centered(SDL_Renderer *r, TTF_Font *font, const char *text, int cx, int cy,
                               SDL_Color color) {
    if (!font || !text)
        return;
    int tw, th;
...
bool view_init_window(void) {
    ...
    g_view.window =
        SDL_CreateWindow("Match-3 Game", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
    ...
    g_view.renderer =
        SDL_CreateRenderer(g_view.window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
    if (!g_view.renderer) {
        /* 退回使用软件渲染兜底 */
        g_view.renderer = SDL_CreateRenderer(g_view.window, -1, SDL_RENDERER_SOFTWARE);
        if (!g_view.renderer) {
...
void view_destroy_window(void) {
    if (g_view.sfx_swap) {
        Mix_FreeChunk(g_view.sfx_swap);
        g_view.sfx_swap = NULL;
    }
    if (g_view.sfx_match) {
        Mix_FreeChunk(g_view.sfx_match);
        g_view.sfx_match = NULL;
    }
    if (g_view.sfx_error) {
        Mix_FreeChunk(g_view.sfx_error);
        g_view.sfx_error = NULL;
    }
    if (g_view.sfx_combo) {
        Mix_FreeChunk(g_view.sfx_combo);
        g_view.sfx_combo = NULL;
    }
    if (g_view.sfx_game_over) {
        Mix_FreeChunk(g_view.sfx_game_over);
        g_view.sfx_game_over = NULL;
    }
    if (g_view.sfx_start) {
        Mix_FreeChunk(g_view.sfx_start);
        g_view.sfx_start = NULL;
    }
    if (g_view.sfx_clear) {
        Mix_FreeChunk(g_view.sfx_clear);
        g_view.sfx_clear = NULL;
    }
    if (g_view.sfx_pao) {
        Mix_FreeChunk(g_view.sfx_pao);
        g_view.sfx_pao = NULL;
    }

    if (g_view.bgm_main) {
        Mix_FreeMusic(g_view.bgm_main);
        g_view.bgm_main = NULL;
    }
    if (g_view.bgm_game) {
        Mix_FreeMusic(g_view.bgm_game);
        g_view.bgm_game = NULL;
    }

    if (g_view.tex_nuist_badge) {
        SDL_DestroyTexture(g_view.tex_nuist_badge);
        g_view.tex_nuist_badge = NULL;
    }
...
void view_unload_assets(void) {
    if (g_view.font_hint) {
        TTF_CloseFont(g_view.font_hint);
        g_view.font_hint = NULL;
    }
    if (g_view.font_body) {
        TTF_CloseFont(g_view.font_body);
        g_view.font_body = NULL;
    }
    if (g_view.font_medium) {
        TTF_CloseFont(g_view.font_medium);
        g_view.font_medium = NULL;
    }
    if (g_view.font_large) {
        TTF_CloseFont(g_view.font_large);
        g_view.font_large = NULL;
    }
    if (g_view.font_title) {
        TTF_CloseFont(g_view.font_title);
        g_view.font_title = NULL;
    }
    ...
    if (g_view.tex_prop_hammer) {
        SDL_DestroyTexture(g_view.tex_prop_hammer);
        g_view.tex_prop_hammer = NULL;
    }
    if (g_view.tex_prop_wand) {
        SDL_DestroyTexture(g_view.tex_prop_wand);
        g_view.tex_prop_wand = NULL;
    }
    if (g_view.tex_prop_shuffle) {
        SDL_DestroyTexture(g_view.tex_prop_shuffle);
        g_view.tex_prop_shuffle = NULL;
    }
    if (g_view.tex_prop_moves) {
        SDL_DestroyTexture(g_view.tex_prop_moves);
        g_view.tex_prop_moves = NULL;
    }
    if (g_view.tex_bg) {
        SDL_DestroyTexture(g_view.tex_bg);
        g_view.tex_bg = NULL;
    }
    if (g_view.tex_bg_main) {
        SDL_DestroyTexture(g_view.tex_bg_main);
        g_view.tex_bg_main = NULL;
    }
    if (g_view.tex_sandglass) {
        SDL_DestroyTexture(g_view.tex_sandglass);
        g_view.tex_sandglass = NULL;
    }
    if (g_view.tex_ice) {
        SDL_DestroyTexture(g_view.tex_ice);
        g_view.tex_ice = NULL;
    }
    if (g_view.tex_dead_end) {
        SDL_DestroyTexture(g_view.tex_dead_end);
        g_view.tex_dead_end = NULL;
    }
    if (g_view.tex_stone) {
        SDL_DestroyTexture(g_view.tex_stone);
        g_view.tex_stone = NULL;
    }
}
...
void view_play_sound_effect(const char *sound_name) {
    ...
    if (strcmp(sound_name, "swap") == 0)
        chunk = g_view.sfx_swap;
    else if (strcmp(sound_name, "match") == 0)
        chunk = g_view.sfx_match;
    else if (strcmp(sound_name, "error") == 0)
        chunk = g_view.sfx_error;
    else if (strcmp(sound_name, "combo") == 0)
        chunk = g_view.sfx_combo;
    else if (strcmp(sound_name, "game_over") == 0)
        chunk = g_view.sfx_game_over;
    else if (strcmp(sound_name, "start") == 0)
        chunk = g_view.sfx_start;
    else if (strcmp(sound_name, "clear") == 0)
        chunk = g_view.sfx_clear;
    else if (strcmp(sound_name, "pao") == 0)
        chunk = g_view.sfx_pao;
...
static void draw_gem(const Gem *g, bool is_selected) {
    ...
    if (g->bomb_type != BOMB_NONE && scale > 0.5f) {
        ...
        if (g->bomb_type == BOMB_RADIUS) {
            draw_rounded_rect_outline(r, b_cx - b_size / 2, b_cy - b_size / 2, b_size, b_size,
                                      b_size / 2, (SDL_Color) {255, 255, 255, 200}, 2);
        }
    }
    ...
    /* 消除闪光圈 */
    if (g->is_marked_for_elimination) {
        draw_rounded_rect_outline(r, x - 1, y - 1, w + 2, h + 2, cr, kMarkRingColor, 2);
    }
}
...
static void draw_info_panel(const GameBoard *board) {
    ...
    if (board->moves_remaining <= 5) {
        if ((SDL_GetTicks64() / 250) % 2 == 0)
            moves_c = kDangerColor;
        else
            moves_c = (SDL_Color) {150, 0, 0, 255};
    }
    ...
    ty += 20;
    draw_text_centered(r, g_view.font_hint, "R 重来", tx, ty, kTextHintColor);
    ty += 28;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (board->difficulty == 2) {
            if (i == 2 && board->used_sandglass_count > 0)
                locked = true;
            if (i == 3 && (board->level & 2))
                locked = true;
        }
        ...
        fill_rounded_rect(r, dx, dy, p_size, p_size, 8,
                          locked ? (SDL_Color) {30, 30, 30, 255} : (SDL_Color) {60, 50, 80, 255});
        if (props[i].tex) {
            if (locked)
                SDL_SetTextureColorMod(props[i].tex, 100, 100, 100);
            SDL_RenderCopy(r, props[i].tex, NULL, &pr);
            if (locked)
                SDL_SetTextureColorMod(props[i].tex, 255, 255, 255);
        }

        if (props[i].count > 0) {
            snprintf(buf, sizeof(buf), "x%u", props[i].count);
            draw_text_centered(r, g_view.font_hint, buf, dx + p_size / 2, dy + p_size + 14,
                               locked ? kTextHintColor : kTextPrimaryColor);
        } else {
            uint32_t price = 0;
            if (i == 0)
                price = 50;
    ...
    if (hovered_prop_idx != -1) {
        static const char *prop_desc[] = {
            "小木槌: 砸碎并消除单个方块", "魔法棒: 强制交换任意两个方块",
            "星空重置: 重新洗牌所有方块", "时光沙漏: 增加 5 步剩余步数"};
        ...
        draw_text_centered(r, g_view.font_hint, text, BOARD_OFFSET_X + (BOARD_WIDTH * GEM_SIZE) / 2,
                           props_y + p_size + 36, (SDL_Color) {200, 200, 255, 255});
    }
}
...
static void draw_combo_popup(const GameBoard *board) {
    ...
    SDL_Color text_col = {255, 214, 10, alpha}; /* 琥珀色 */
...
void view_draw_game_ui_complete(const GameBoard *board) {
    ...
    for (int row = 0; row < BOARD_HEIGHT; row++) {
        for (int col = 0; col < BOARD_WIDTH; col++) {
            ...
            bool sel = board->first_gem_selected && row == board->selected_row &&
                       col == board->selected_col;

            bool hover = (board->current_state == GAME_STATE_WAITING_INPUT ||
                          board->current_state == GAME_STATE_FIRST_GEM_SELECT) &&
                         row == board->hover_row && col == board->hover_col;

            if (board->has_hint) {
                bool is_hint =
                    (row == board->hint_r && col == board->hint_c) ||
    ...
    for (int i = 0; i < MAX_PARTICLES; i++) {
        if (g_particles[i].active) {
            ...
            fill_rounded_rect(r, (int) g_particles[i].x, (int) g_particles[i].y, p_size, p_size, 1,
                              pc);
        }
    }
...
static void draw_menu_button(int cx, int y, int w, int h, const char *label, bool selected,
                             const char *sub_label, bool locked) {
    SDL_Renderer *r = g_view.renderer;

    SDL_Color fill =
        selected ? kAccentBlueColor : (locked ? (SDL_Color) {20, 20, 20, 255} : kPanelColor);
    SDL_Color border =
        selected ? kAccentBlueColor : (locked ? (SDL_Color) {40, 40, 40, 255} : kPanelBorderColor);
    int bw = selected ? 2 : 1;
    ...
    if (sub_label) {
        SDL_Color sc = selected ? (SDL_Color) {200, 220, 255, 255}
                                : (locked ? (SDL_Color) {80, 80, 80, 255} : kTextSecondaryColor);
...
void view_draw_main_menu(const GameBoard *board) {
    ...
    if (g_view.tex_nuist_badge) {
        ...
        float scale = 1.0f;
        if (w > 120)
            scale = 120.0f / (float) w;
        if ((float) h * scale > 120.0f)
            scale = 120.0f / (float) h;
        SDL_Rect dst = {cx - (int) ((float) w * scale) / 2, 40, (int) ((float) w * scale),
                        (int) ((float) h * scale)};
        SDL_RenderCopy(r, g_view.tex_nuist_badge, NULL, &dst);

        draw_text_centered(r, g_view.font_title, "消消乐", cx, 200, kTextPrimaryColor);
        draw_text_centered(r, g_view.font_medium, "按任意键或点击继续", cx, 260,
                           kTextSecondaryColor);
        ...
        draw_text_centered(r, g_view.font_medium, "经典三消游戏体验", cx, 160, kTextSecondaryColor);
    ...
    draw_text_centered(r, g_view.font_hint, "方向键选择  |  Enter确认  |  ESC退出", cx,
                       WINDOW_HEIGHT - 30, kTextHintColor);
}
...
static void view_draw_rules(const GameBoard *board) {
    ...
    const char *rules[]   = {"【基础玩法】消除3个同色宝石。可用金币购买下方道具破局。",
                             "【困难障碍】石块为永久障碍；冰块会冻结宝石并阻挡下落，",
                             "            须通过消除相邻宝石或使用小木槌等道具打碎。",
                             "",
                             "【死局机制与复活】",
                             "如果在未耗尽步数时，盘面上没有任何可进行的消除（死局），",
                             "游戏将判定为结束。在结束界面，你可以选择消耗1个【星空重置】",
                             "道具或花费金币进行复活，系统会为你重新洗牌并额外赠送 3 步。",
                             "",
                             "【道具使用限制说明】（常见问题）",
                             "如果你的道具图标全部变暗且无法使用，是因为触发了以下限制：",
                             "1. 总次数限制：单局使用的所有道具总数有上限。",
                             "   （简单模式无限制；普通模式最多 5 次；困难模式最多 3 次）",
                             "2. 互斥限制：在困难模式下，【星空重置】和【时光沙漏】非常",
                             "   强大，因此不仅每局各限用 1 次，而且彼此互斥。只要",
                             "   使用了其中一个，另一个就会立刻被锁定，本局无法再使用。"};
    int         num_rules = sizeof(rules) / sizeof(rules[0]);

    int start_y = 150;
    for (int i = 0; i < num_rules; i++) {
        draw_text_centered(r, g_view.font_medium, rules[i], cx, start_y + i * 35,
                           kTextSecondaryColor);
    }
...
void view_draw_difficulty_menu(const GameBoard *board) {
    ...
    draw_text_centered(r, g_view.font_medium, "不同的挑战与奖励", cx, 140, kTextSecondaryColor);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        bool locked = false;
        draw_menu_button(cx, 220 + i * 100, 300, 74, NAMES[i], sel, locked ? "未解锁" : MOVES[i],
                         locked);
    }

    draw_text_centered(r, g_view.font_hint, "方向键选择  |  Enter确认  |  ESC返回", cx,
                       WINDOW_HEIGHT - 30, kTextHintColor);
}
...
void view_draw_game_over_screen(const GameBoard *board) {
    ...
    draw_text_centered(r, g_view.font_hint, "方向键选择，回车确认", cx, WINDOW_HEIGHT - 30,
                       kTextHintColor);
}
...
bool view_render_frame(const GameBoard *board) {
    ...
    if (board->current_state == GAME_STATE_MAIN_MENU ||
        ...
        board->current_state == GAME_STATE_GAME_OVER || board->current_state == GAME_STATE_RULES) {
        if (g_view.tex_bg_main) {
